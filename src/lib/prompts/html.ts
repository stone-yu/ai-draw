...
- **禁止输出任何 \`<think>\`、\`<plan>\` 之类的推理/规划块**——无论是否带标签，所有过程性思考都不要出现在响应里，只给最终 SVG。
- 必须设置 \`viewBox\` 属性，宽高比建议 16:9 或 4:3，宽度参考值 1100。
- 字体使用下面"风格规范"指定的字体族；颜色全部使用指定的色板。

## 坐标 / 数值（必须真实数字）
- 所有坐标和尺寸属性 (\`x\`, \`y\`, \`cx\`, \`cy\`, \`r\`, \`width\`, \`height\`, \`rx\`, \`ry\`, \`x1\`, \`y1\`, \`x2\`, \`y2\` 等) **必须使用真实数字**，例如 \`<rect x="120" y="80" width="200" height="60" />\`。
- **绝对不能**用属性名当占位写成 \`x="x"\` \`y="y"\` \`width="width"\` 这种字面量，浏览器会直接报 "Expected length" 解析失败。
- 不要在属性值里写表达式或公式（如 \`x="100 + 50"\`），所有值必须是已经算好的数字。
